...
Version: 0.1.1
Author: Riyad Arefin
Author URI: http://pressply.com
*/

if ( !class_exists( 'riyadtitle' ) ) {

    class riyadtitle {

    	private $slug = 'riyad_title';
...
<!-- END -->
		...
		public function add_box(){
			...
			foreach ( $posttypes as $posttype ){
				add_meta_box( $this->slug, 'WP Hide Title', array( $this, 'build_box' ), $posttype, 'side' );
			}
		...
		public function build_box( $post ){
			...
			?>
			<label><input type="checkbox" name="<?php echo $this->slug; ?>" <?php echo $checked; ?> /> Want to hide the title? </label>
			<?php
    ...
    $riyadtitle = new riyadtitle;

} // !class_exists( 'riyadtitle' )